(async () => {
  ...
  function isTargetNode(node: SceneNode): boolean {
    return (
      node.type === "FRAME" ||
      node.type === "GROUP" ||
      node.type === "COMPONENT" ||
  ...
  function createSkeletonRectangle(node: SceneNode, parent: FrameNode | GroupNode, offsetX: number, offsetY: number) {
  ...
  function containsExactlyOneVector(node: SceneNode): boolean {
    if ('children' in node) {
  ...
  function traverseNode(node: SceneNode, parent: FrameNode | GroupNode, offsetX: number, offsetY: number) {
